...
项目采用统一的暖绿色调，体现医疗健康感，全站保持视觉一致性。

| 用途 | 色值 | 说明 |
|------|------|------|
| 主色调 | `#52b788` | 暖绿色，用于品牌标识、主按钮、激活状态 |
| 主色深 | `#40916c` | 深绿色，用于按钮 hover、强调 |
| 侧边栏背景 | `#1b4332` | 深森绿，后台侧边栏背景 |
| 侧边栏渐变 | `#2d6a4f → #40916c → #52b788` | 登录页左侧品牌区域 |
| 强调色 | `#f4845f` | 暖橙色，用于 CTA 按钮（预约挂号等） |
| 辅助色 | `#48cae4` | 天蓝色，用于信息提示 |
| 页面背景 | `#fafaf5` | 浅暖灰，门户页面背景 |
| 内容区背景 | `#f0f2f5` | 浅灰色，后台内容区背景 |
| 主文字 | `#2d3436` | 深灰色，正文文字 |
| 次文字 | `#636e72` | 中灰色，辅助说明文字 |
| Footer 背景 | `#2d3436` | 深灰色，门户底部栏 |

#### 3.5.2 页面风格

| 页面 | 风格说明 |
|------|----------|
| 门户首页 (`/`) | 暖绿渐变 Hero + 白底卡片 + 深灰 Footer |
| 登录页 (`/login`) | 浅绿渐变背景 + 左右分栏卡片（左品牌信息、右登录表单） |
| 管理后台 (`/console/*`) | 深森绿侧边栏 + 白色顶栏 + 浅灰内容区 |

#### 3.5.3 交互规范

- 按钮 hover 颜色加深一级（`#52b788 → #40916c`）
- 卡片 hover 上浮 6px（`transform: translateY(-6px)`）
- 菜单激活项使用主色调背景（`#52b788`）
- 菜单 hover 使用半透明主色调（`rgba(82, 183, 136, 0.3)`）
- 页面切换过渡平滑（侧边栏折叠 `transition: width 0.3s`）

#### 3.5.4 响应式断点

- 小屏幕 `≤768px`：登录页隐藏左侧品牌区，服务卡片单列
- 中屏幕 `≤1024px`：服务卡片两列
- 大屏幕 `>1024px`：服务卡片四列，内容区最大宽度 1200px
...
| 2026-02-26 | v26.2.4 | 添加 UI 设计规范章节（配色方案、页面风格、交互规范、响应式断点） | - |